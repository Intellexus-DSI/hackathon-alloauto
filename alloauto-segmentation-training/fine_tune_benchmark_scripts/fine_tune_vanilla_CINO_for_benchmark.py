...
import pandas as pd
import os
# need python -m pip install -U sentencepiece
...
def train_CINO_baseline():
    ...
    OUTPUT_DIR = './alloauto-segmentation-training/benchmark_models/CINO_baseline_model_clean_train'
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_name,
        # use_fast=False,  # <— avoid the Tiktoken conversion path entirely
        # add_prefix_space=True,  # <— required for (XLM-)RoBERTa with is_split_into_words=True
    )
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="steps",  # <— was eval_strategy
        eval_steps=30,
        save_strategy="steps",
        save_steps=60,
        learning_rate=1e-5,
...
if __name__ == "__main__":
    trainer, model, tokenizer = train_CINO_baseline()